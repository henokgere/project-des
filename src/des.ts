export class DES {
    private subkeys: string[]; // Array to store the 16 round subkeys as binary strings
  
    constructor(key: string) {
      // Generate subkeys from the input key (now a binary string)
    ...
    private crypt(input: string, decrypt: boolean): string {
      const blocks = Math.ceil(input.length / 64); // Calculate number of 64-bit blocks
      let output = '';
  
      for (let i = 0; i < blocks; i++) {
        // Process each 64-bit block
        const block = input.slice(i * 64, (i + 1) * 64).padEnd(64, '0');
  
        let data = this.initialPermutation(block);
        ...
        for (let round = 0; round < 16; round++) {
          ...
          const temp = left;
          left = right;
          right = this.xor(temp, this.feistel(right, subkey));
        }
  
        data = right + left; // Combine left and right halves
        output += this.finalPermutation(data);
      }
    ...
    private feistel(right: string, subkey: string): string {
      const expanded = this.expand(right);
      ...
      for (let i = 0; i < 8; i++) {
        ...
        result += S_BOXES[i][row][col].toString(2).padStart(4, '0');
      }
  
      return this.permute(result, P);
    }
  
    // Generate 16 round subkeys from the initial key
    private generateSubkeys(key: string): string[] {
      const subkeys: string[] = [];
      let permutedKey = this.permute(key, PC1);
      ...
      for (let i = 0; i < 16; i++) {
        ...
        subkeys.push(this.permute(combined, PC2));
      }
    ...
    private expand(input: string): string {
      return this.permute(input, E);
    }
  
    // Initial Permutation (IP in DES)
    private initialPermutation(input: string): string {
      return this.permute(input, IP);
    }
  
    // Final Permutation (FP in DES)
    private finalPermutation(input: string): string {
      return this.permute(input, FP);
    }
    ...
    // Static method to pad input to multiple of 64 bits
    static padInput(input: string): string {
      const blockSize = 64;
      const padding = blockSize - (input.length % blockSize);
      return input.padEnd(input.length + padding, '0');
    }
  
    // Static method to remove padding from decrypted data
    static unpadInput(input: string): string {
